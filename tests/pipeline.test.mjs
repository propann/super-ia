...
function processResult(runId, cwd) {
  return {
    ...
    timedOut: false,
    durationMs: 10,
    stdoutPath: join(cwd, `${runId}.stdout.log`),
    stderrPath: join(cwd, `${runId}.stderr.log`),
    stdoutBytes: 0,
    stderrBytes: 0,
    truncated: false,
    status: "completed",
...
function execution(provider, mode, cwd, id, runId, lastMessagePath) {
  ...
  return {
    ...
    sandboxPreflight: {
      ...
      engine: "bubblewrap",
      network: "isolated",
      workspaceAccess: mode === "build" ? "read-write" : "read-only",
      ephemeralHome: true,
    },
    process: processResult(runId, cwd),
    lastMessagePath,
    normalizedEventsPath: join(ctx.directory, "EVENTS.json"),
    parsedEvents: 1,
    invalidEventLines: 0,
    changeGuard: {
      schemaVersion: 1,
      passed: true,
      allowedPaths: mode === "build" ? ["src/**"] : [],
      changedFiles: mode === "build" ? ["src/app.js"] : [],
      outOfScopeFiles: [],
      diffPath: join(ctx.directory, "AGENT_CHANGES.patch"),
      reportPath: join(ctx.directory, "CHANGE_GUARD.json"),
    },
  };
}
...
test("pipeline enforces independent builder, validations, review and receipt", async () => {
  ...
  try {
    ...
    const codex = async (_root, _taskId, options) => {
      ...
      return execution("codex-cli", "build", updated.worktreePath, "CTX-BUILDER", "RUN-BUILDER", message);
    };
    ...
    const receipt = async (runId) => {
      ...
      return {
        ...
        receipt: {
          ...
          git: { cwd: updated.worktreePath, dirty: true, changedFiles: [], diffSha256: "hash" },
          artifacts: [],
          validations: [],
    ...
    const result = await runControlledPipeline(root, task.id, {
      builder: "codex",
      reviewer: "vibe",
      timeoutMs: 30_000,
    }, {
      codex,
      vibe,
      validate: async (directory, options) => {
        order.push("validate");
        return runRepositoryValidations(directory, options);
      },
      receipt,
    });
...
test("pipeline refuses the same provider before execution", async () => {
  await assert.rejects(
    runControlledPipeline(".", "TASK-0001", { builder: "codex", reviewer: "codex" }),
    /différent/,
  );
});
