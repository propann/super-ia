...
import { loadConnectionStore } from "../connections/store.js";
import { ensureMachineStore } from "../machines/store.js";
import { ensureArenaState } from "../arena/store.js";
...
function restoreDestination(root: string, name: string): string {
  ...
  if (name === "connections.json") return join(root, "connections.json");
  if (name === "machines.json") return join(root, "machines.json");
  if (name === "arena.json") return join(root, "arena.json");
...
export async function createControlBackup(
  ...
  const optional = await Promise.all([
    ...
    copyOptionalPrivateFile(join(paths.root, "connections.json"), join(directory, "connections.json")),
    copyOptionalPrivateFile(join(paths.root, "machines.json"), join(directory, "machines.json")),
    copyOptionalPrivateFile(join(paths.root, "arena.json"), join(directory, "arena.json")),
  ]);
...
export async function restoreControlBackup(
  ...
  try {
    ...
    if (verification.manifest.files.some((file) => file.name === "connections.json")) {
      await loadConnectionStore(staging);
    }
    if (verification.manifest.files.some((file) => file.name === "machines.json")) {
      await ensureMachineStore(staging);
    }
    if (verification.manifest.files.some((file) => file.name === "arena.json")) {
      await ensureArenaState(staging);
    }
