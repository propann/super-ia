...
let consoleStream;let activeConsoleId='';
function appendConsole(value){const output=el('console-output');output.textContent=(output.textContent+value).slice(-20000);output.scrollTop=output.scrollHeight;}
async function sendConsoleInput(){const input=el('console-input');const data=input.value+'\n';if(!activeConsoleId||!input.value)return;input.value='';await fetch('/api/console/'+encodeURIComponent(activeConsoleId)+'/input',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({data})});}
async function openConsole(id){const machine=(window.arenaData?.machines||[]).find((item)=>item.id===id);if(!machine)return;activeConsoleId=id;el('console-title').textContent=machine.label||id;el('console-meta').textContent=String(machine.platform||'machine').toUpperCase()+' · '+String(machine.transport||'ssh').toUpperCase();el('console-output').textContent=consoleLines(machine).join('\n')+'\n';el('console-drawer').classList.add('open');try{const opened=await fetch('/api/console/'+encodeURIComponent(id)+'/open',{method:'POST',headers:{'Accept':'application/json'}});if(!opened.ok)throw new Error((await opened.json()).error||'Ouverture SSH refusée');consoleStream=new EventSource('/api/console/'+encodeURIComponent(id)+'/stream');consoleStream.addEventListener('history',(event)=>{el('console-output').textContent=event.data?JSON.parse(event.data):'';});consoleStream.addEventListener('output',(event)=>appendConsole(JSON.parse(event.data)));}catch(error){appendConsole('\n[SUPER IA] '+(error instanceof Error?error.message:String(error))+'\n');}}
function closeConsole(){if(consoleStream){consoleStream.close();consoleStream=undefined;}if(activeConsoleId)void fetch('/api/console/'+encodeURIComponent(activeConsoleId)+'/close',{method:'POST'});activeConsoleId='';el('console-drawer').classList.remove('open');}
...
function dragGroup(event,index){event.dataTransfer.setData('text/superia-group',String(index));event.dataTransfer.effectAllowed='move';}
function dropOnGroup(event,index){event.preventDefault();const groupIndex=event.dataTransfer.getData('text/superia-group');if(groupIndex!==''){const source=Number(groupIndex);if(Number.isInteger(source)&&source!==index){const groups=arenaState.groups.map((group)=>[...group]);groups[index]=[...new Set([...groups[index],...groups[source])];groups.splice(source,1);arenaState.groups=groups;arenaState.selected=[];saveArena();renderArena(window.arenaData||{});}return;}const source=event.dataTransfer.getData('text/superia-entity');if(!source)return;const groups=arenaState.groups.map((group)=>[...group]);const sourceIndex=groups.findIndex((group)=>group.includes(source));if(sourceIndex>=0&&sourceIndex!==index){groups[index]=[...new Set([...groups[index],...groups[sourceIndex])];groups.splice(sourceIndex,1);}else if(!groups[index].includes(source)){groups[index].push(source);}arenaState.groups=groups;arenaState.selected=[];saveArena();renderArena(window.arenaData||{});}
function groupSelected(){const selected=[...new Set(arenaState.selected)];if(selected.length<2)return;const chosen=new Set(selected);arenaState.groups=arenaState.groups.map((group)=>group.filter((key)=>!chosen.has(key))).filter((group)=>group.length>=2);arenaState.groups.push(selected);arenaState.selected=[];saveArena();renderArena(window.arenaData||{});}
...
function renderArena(data){
  ...
  el('groups').innerHTML=arenaState.groups.map((group,index)=>'<div class="group-chip" draggable="true" ondragstart="dragGroup(event,'+index+')" ondragover="event.preventDefault()" ondrop="dropOnGroup(event,'+index+')"><span><strong>GROUPE '+(index+1)+'</strong><br>'+group.map((key)=>esc(all.get(key)?.label||all.get(key)?.name||key)).join(' ↔ ')+'</span><button class="btn secondary" type="button" onclick="event.stopPropagation();dissolveGroup('+index+')">DISSOUT</button></div>').join('')||'<span class="sub">Aucun groupe isolé</span>';
}
...
export function renderDashboardPage(): string {
  ...
  <div class="footer">État en lecture · consoles SSH contrôlables avec clé ou agent SSH · arrêt d'urgence via CLI · boucle locale · aucune CORS · rafraîchissement 30 s</div>
