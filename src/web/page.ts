...
function renderArena(data){
  ...
  const machineCards=machines.map((x)=>'<button draggable="true" ondragstart="dragEntity(event,\'machine\',\''+esc(x.id)+'\')" ondragover="event.preventDefault()" ondrop="dropEntity(event,\'machine\',\''+esc(x.id)+'\')" class="entity-card '+(arenaState.selected.includes(entityKey('machine',x.id))?'selected':'')+'" type="button" onclick="selectEntity(\'machine\',\''+esc(x.id)+'\')"><span class="entity-icon">⌘</span><span class="entity-name">'+esc(x.label)+'</span><span class="entity-meta">'+esc(x.platform)+' · '+esc(x.state)+'</span></button>').join('');
