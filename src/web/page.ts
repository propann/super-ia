...
async function load(projectId){
  ...
  try{
    ...
    el('cards').innerHTML=[
      ['Projets',data.status.projects],['Missions',data.status.tasks],['Runs',data.status.runs],['Runs actifs',data.status.activeRuns],['Notifications',data.notifications.length],['Readiness',data.readiness?.overall || 'indisponible']
...
export function renderDashboardPage(): string {
  ...
  <div class="footer">Lecture seule · boucle locale · aucune CORS · rafraîchissement 30 s</div>
